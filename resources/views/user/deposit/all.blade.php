...
<div class="page-content">
    ...
    <tr>
        ...
        <td>
            @if($item->status == 'pending')
                <a href="#" class="btn btn-info sm" title="Edit Data" onclick="openPaymentModal('{{ $item->payment_proof }}', '{{ $item->amount }}', '{{ $item->user->email }}', '{{ $item->feedback }}','{{ $item->id }}')"><i class="fa fa-pencil-alt"></i></a>
            @else
                <a href="#" class="btn btn-info sm" title="View Data" onclick="openPaymentModal('{{ $item->payment_proof }}', '{{ $item->amount }}', '{{ $item->user->email }}','{{ $item->feedback }}', '{{ $item->id }}')"><i class="fas fa-eye"></i></a>
            @endif
        </td>
    </tr>
@endforeach
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <img id="paymentProof" src="" alt="Evidence of Payment" class="img-fluid mb-3">
...
<script>
    function openPaymentModal(paymentProofSrc, amount, userEmail,feedBack, depositId) {
        var imageUrl = '{{ asset("storage/") }}/' + paymentProofSrc; // Prepend base URL to image path
        document.getElementById('paymentProof').src = imageUrl;
